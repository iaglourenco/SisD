...
def main(s,port):
    ...
    while True:
        ...
        if identity == 'dnsack': # DNS Connection
            Thread(target=server_thread,args=(ns,address,)).start()
        ...
        if command == 'propa' or command == 'inser':
            ...
            if  time.time() - time_count < 60:      
                if user_count < 2:
                    ...
                    tmp_s.listen(5)
                    Thread(target=main,args=(tmp_s,tmp_port,)).start()
...
if __name__ == '__main__':
    ...
    ap.add_argument('--dns',help='create a new dns server, on the port informed',required=False,action='store_true')
    ap.add_argument('-p','--port',help='the port to execute the script on',required=False,type=int)
